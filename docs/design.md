...
媒体增强采用条目驱动的直接关联，避免通过后代文件反推任意祖先目录：

```text
folder.jpg / poster.jpg  -> 所在文件夹的 artwork / folder 关联
backdrop.jpg / fanart.jpg -> 所在文件夹的 backdrop 关联
movie.nfo / tvshow.nfo   -> 所在文件夹的电影或剧集信息
视频文件                  -> 文件名解析 + TMDB 匹配，写入视频文件自身
EPUB 等图书文件           -> 解析文件元数据，写入图书文件自身
```

对于 Movies / TV Shows 这类“一层目录代表一个作品”的资料库，只有已经明确
识别为电影或剧集的文件，才将同一 MediaItem 投影到它所属的资料库顶层文件夹，
用于文件夹标题、封面和背景展示。资料库源根目录只是容器，不能关联为某一个
MediaItem。本地 NFO 和显式锁定结果优先于自动匹配；缺少可靠结果时保留普通
文件图标，不通过降低匹配阈值或修改源数据来制造封面。
